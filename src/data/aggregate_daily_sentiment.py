...
def build_daily_features(g: pd.DataFrame) -> pd.DataFrame:
    ...
    daily['sentiment_mean_3d'] = daily['sentiment_final_mean'].rolling(3, min_periods=1).mean()
    daily['news_count_3d'] = daily['news_count'].rolling(3, min_periods=1).sum()
    daily['sentiment_ema_7d'] = daily['sentiment_final_mean'].ewm(span=7, adjust=False).mean()

    prior_mean = daily['sentiment_final_mean'].shift(1).rolling(7, min_periods=1).mean()
    daily['sentiment_trend_7d'] = (daily['sentiment_final_mean'] - prior_mean).fillna(0.0)

    # Tambahan fitur yang sebelumnya belum ada
    daily['sentiment_delta_1d'] = daily['sentiment_final_mean'].diff().fillna(0.0)

    # Sinyal arah sederhana dan aman: -1, 0, 1
    daily['sentiment_dir_signal'] = 0
    daily.loc[daily['sentiment_final_mean'] > 0, 'sentiment_dir_signal'] = 1
    daily.loc[daily['sentiment_final_mean'] < 0, 'sentiment_dir_signal'] = -1

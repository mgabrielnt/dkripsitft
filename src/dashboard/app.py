...
COUNT_COLUMNS = {"n", "ndiracc", "count", "jumlah", "split", "index", "unnamed0"}
...
def identify_metric_columns(df):
    ...
    if metric_cols:
        return metric_cols
    identifier_cols = [find_col(df, MODEL_CANDIDATES), find_col(df, HORIZON_CANDIDATES), find_col(df, TICKER_CANDIDATES)]
    identifier_cols = [col for col in identifier_cols if col]
    return [col for col in df.select_dtypes(include="number").columns if col not in identifier_cols and norm(col) not in COUNT_COLUMNS]
...
def detect_model_col(df):
    ...
    for candidate in df.columns:
        if df[candidate].dtype == "object":
            values = " ".join(df[candidate].dropna().astype(str).head(20).tolist()).lower()
...
def add_model_if_missing(df, scope="global"):
    ...
    elif scope == "horizon" and len(out) % 3 == 0 and len(out) >= 6:
        out["Model"] = ["TFT"] * (len(out) // 2) + ["LLM-TFT"] * (len(out) - len(out) // 2)
    else:
        out["Model"] = "Model"
    return out, "Model"
...
def prepare_long_eval(df, scope="global"):
    ...
    if metric_name_col and value_col:
        work, model_col = add_model_if_missing(work, scope)
        rows = []
        for _, row in work.iterrows():
            rows.append({
                "Model": row.get(model_col, "Model"),
                "Metric": metric_display_name(row.get(metric_name_col)),
                "Value": pd.to_numeric(row.get(value_col), errors="coerce"),
                "Horizon": row.get(find_col(work, HORIZON_CANDIDATES), None),
                "Ticker": row.get(find_col(work, TICKER_CANDIDATES), None),
            })
    ...
    for _, row in work.iterrows():
        for metric_col in metric_cols:
            ...
            rows.append({
                "Model": row.get(model_col, "Model"),
                "Metric": metric_display_name(metric_col),
                "Value": value,
                "Horizon": row.get(horizon_col, None) if horizon_col else None,
                "Ticker": row.get(ticker_col, None) if ticker_col else None,
            })
    long_df = pd.DataFrame(rows)
    if long_df.empty:
        return long_df
    long_df["Model"] = long_df["Model"].apply(normalize_model)
    if scope == "horizon" and long_df["Horizon"].isna().all():
        horizons = ["H+1", "H+2", "H+3"]
        unique_rows = work.shape[0]
        repeated = []
        for i in range(unique_rows):
            repeated.append(horizons[i % 3])
        mapping = dict(zip(range(unique_rows), repeated))
    return long_df
...
def show_eval_section(title, df, scope, x_field=None):
    ...
    if long_df.empty:
        st.info(f"File {title.lower()} belum dapat dibaca sebagai grafik, tetapi dashboard tidak lagi berhenti karena perbedaan nama kolom.")
        return
    if x_field and x_field in long_df.columns and long_df[x_field].notna().any():
        x_axis = x_field
    else:
        x_axis = "Model"
    metrics = [m for m in ["RMSE", "MAE", "MAPE", "R²", "Directional Accuracy"] if m in long_df["Metric"].unique().tolist()]
    other_metrics = [m for m in long_df["Metric"].unique().tolist() if m not in metrics]
    ...
    for idx, metric in enumerate(metrics):
        ...
        with cols[idx % 2]:
            if x_axis == "Model":
                fig = px.bar(metric_df, x="Model", y="Value", color="Model", title=f"{title} - {metric}", color_discrete_map=MODEL_COLORS, text_auto=True)
            elif scope == "horizon":
                fig = px.line(metric_df, x=x_axis, y="Value", color="Model", markers=True, title=f"{title} - {metric}", color_discrete_map=MODEL_COLORS)
                fig.update_traces(line=dict(width=3), marker=dict(size=9))
            else:
                fig = px.bar(metric_df, x=x_axis, y="Value", color="Model", barmode="group", title=f"{title} - {metric}", color_discrete_map=MODEL_COLORS, text_auto=True)
...
def attention_dataframe(df):
    ...
    return pd.DataFrame({
        ...
        "Attention Weight": [54, 62, 68, 72, 75, 78, 81, 83, 85, 87, 90, 93, 96, 98, 101] + [86, 79, 76, 75, 76, 78, 80, 82, 83, 84, 84, 85, 85, 86, 86],
...
with st.sidebar:
    ...
    st.divider()
    page = st.radio("Menu Dashboard", ["Model, Prediksi, dan Evaluasi", "Data Harga Saham", "Berita Keuangan dan Sentimen"])
    st.divider()
...
if page == "Model, Prediksi, dan Evaluasi":
    ...
    with st.expander("Jalankan Proses"):
        ...
        with b4: action_button("Evaluasi", "Evaluasi model")
        with b5: action_button("Backtest", "Backtest model")
        with b6: action_button("Interpretasi", "Interpretasi model")
    ...
    if pred_rows:
        ...
        fig = px.bar(pred_plot, x="Horizon", y="Prediksi", color="Model_Normalized", barmode="group", title="Perbandingan Prediksi TFT dan LLM-TFT", color_discrete_map=MODEL_COLORS, text_auto=True)
        st.plotly_chart(layout(fig, 420), use_container_width=True)

...
def read_csv(path):
    ...
    drop_cols = [col for col in df.columns if norm(col) in COUNT_COLUMNS]
...
def identify_metric_columns(df):
    ...
    identifiers = [
        detect_model_col(df),
        find_col(df, HORIZON_CANDIDATES),
        find_col(df, TICKER_CANDIDATES),
    ]
    identifiers = [col for col in identifiers if col]
    return [
        col
        for col in df.select_dtypes(include="number").columns
        if col not in identifiers and norm(col) not in COUNT_COLUMNS
    ]
...
def prepare_long_eval(df, scope="global"):
    ...
    if metric_name_col and value_col:
        ...
        for _, row in work.iterrows():
            rows.append(
                {
                    "Model": normalize_model(row.get("Model", "Model")),
        ...
        for horizon, value in zip(["H+1", "H+2", "H+3"], values):
            if value is not None:
                rows.append(
                    {
                        "Model": model_name,
                        "Model_Normalized": model_name.replace(" S5", "").replace(" S1", ""),
                        "Horizon": horizon,
                        "Prediksi": value,
                    }
                )
...
def show_prediction_panel(prediction_df):
    st.subheader("Prediksi Harga H+1 sampai H+3")

    if prediction_df is None or prediction_df.empty:
        st.info("Prediksi H+1 sampai H+3 belum tersedia.")
        return
    ...
    latest_llm = prediction_df[prediction_df["Model_Normalized"].eq("LLM-TFT")]
    if latest_llm.empty:
        latest_llm = prediction_df

    values = latest_llm.set_index("Horizon")["Prediksi"].to_dict()
    summary = " | ".join(
        [f"{h}: Rp {fmt(values[h])}" for h in ["H+1", "H+2", "H+3"] if h in values]
    )
    if summary:
        st.caption(summary)
...
if page == "Model, Prediksi, dan Evaluasi":
    ...
    show_prediction_panel(prediction_df)

    st.divider()
    st.subheader("Evaluasi Model")
    tab_global, tab_horizon, tab_ticker, tab_attention = st.tabs(
        ["Global", "Horizon", "Emiten", "Attention"]
    )

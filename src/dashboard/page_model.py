...
def render(data, ticker, dates):
    ...
    cutoff = pd.to_datetime(master[date_col(master)]).max() if master is not None and date_col(master) else None
    ...
    latest_close = None
    if master is not None and not master.empty and "close" in master.columns:
        close = master.sort_values(date_col(master) or "time_idx")["close"].dropna()
        latest_close = float(close.tail(1).iloc[-1]) if not close.empty else None
    cards = st.columns(4)

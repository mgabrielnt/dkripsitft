...
OUT_DIR = Path(r'D:/skripsi/tft/modelssss/hybrid')
CAT_COLS = ['ticker', 'day_of_week', 'month', 'is_month_end']
REAL_COLS = [
    'close', 'volume', 'log_return_1d', 'log_return_2d',
    'vol_20', 'rsi_14', 'ma_5_div_ma_20', 'bb_width_20',
...
def rebuild_time_idx_if_needed(df: pd.DataFrame, time_idx_col: str) -> pd.DataFrame:
    ...
    if df[time_idx_col].isna().any():
        ...
        for _, g in df.groupby('ticker', sort=True):
            vals = pd.to_numeric(g[time_idx_col], errors='coerce')
            if vals.isna().any() or not vals.is_monotonic_increasing or not vals.diff().dropna().eq(1).all():
                need_rebuild = True
                break
    if need_rebuild:
...
def prepare_df(path: str, time_idx_col: str, target_col: str) -> pd.DataFrame:
    df = pd.read_csv(path)
    ensure_cols(df, ['split', *CAT_COLS, time_idx_col, target_col], '[HYBRID]')

    if 'date' in df.columns:
        df['date'] = pd.to_datetime(df['date'], errors='coerce')
    else:
        raise ValueError('[HYBRID] kolom date wajib ada agar time_idx bisa diaudit ulang.')
    ...
    if forbidden:
        raise ValueError(f'[HYBRID] ticker terlarang masih ada: {sorted(forbidden)}')
    ...
    df = rebuild_time_idx_if_needed(df, time_idx_col)

    close = pd.to_numeric(df['close'], errors='coerce').replace(0, np.nan)
    sent = pd.to_numeric(df.get('sentiment_final_mean', 0.0), errors='coerce').fillna(0.0)
    if 'log_return_1d' not in df.columns:
        df['log_return_1d'] = np.log(close / close.groupby(df['ticker']).shift(1))
    if 'log_return_2d' not in df.columns:
        df['log_return_2d'] = np.log(close / close.groupby(df['ticker']).shift(2))
    if 'sentiment_delta_1d' not in df.columns:
        df['sentiment_delta_1d'] = sent.groupby(df['ticker']).diff().fillna(0.0)

    ensure_cols(df, REAL_COLS, '[HYBRID features]')
    train_mask = df['split'] == 'train'
    for c in [*REAL_COLS, target_col]:
        df[c] = pd.to_numeric(df[c], errors='coerce').replace([np.inf, -np.inf], np.nan)
        train_med = df.loc[train_mask, c].median()
        fill = 0.0 if pd.isna(train_med) else float(train_med)
...
def main():
    ...
    result.to_csv(OUT_DIR / 'hybrid_val_loss_results.csv', index=False)
